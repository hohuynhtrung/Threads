...
function PostActions({ post }) {
  ...
  return (
    <div className="flex items-center gap-5 pt-2 text-[#4d4d4d]">
      {actions.map((action) => (
        <button
          key={action.id}
          onClick={action.onClick}
          className="flex items-center p-2 rounded-2xl gap-1.5 text-sm font-normal hover:bg-gray-100 transition-colors active:scale-95 cursor-pointer"
        >
          <img
            src={action.icon}
            alt={action.label}
